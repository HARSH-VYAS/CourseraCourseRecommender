...
=======
	...
	public String helloLinkedIn(Model model) {
		...
		model.addAttribute(linkedIn.profileOperations().getUserProfile());
		List<LinkedInProfile> connections = linkedIn.connectionOperations()
				.getConnections();
		model.addAttribute("connections", connections);
	...
	public String courses(Model model) throws RestClientException {
		...
		RestTemplate restTemplate = new RestTemplate();
		Courses courses = restTemplate.getForObject(
	...
	public String coursesMatch(Model model) {
		...
		List<Course> elements = cr.findAll();
		List<String> skills = linkedIn.profileOperations().getUserProfileFull()
				.getSkills();
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			for (int j = 0; j < skills.size(); j++) {
				if (elements.get(i).getShortName()
						.equalsIgnoreCase(skills.get(j))) {
		...
		/*
		 * RestTemplate restTemplate = new RestTemplate(); Courses courses =
		 * restTemplate
		 * .getForObject("https://api.coursera.org/api/catalog.v1/courses",
		 * Courses.class); ArrayList<Course> elements = courses.getElements();
		 * 
		 * cr.save(elements); model.addAttribute("courses", elements);
		 */
